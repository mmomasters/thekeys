...
- **Outgoing Smoobu API calls are HMAC-signed** (the plain `Api-Key` header is deprecated, sunset 2026-09-25). Every request sends `X-API-Key`/`X-Timestamp`/`X-Nonce`/`X-Signature`, where the signature is `base64(HMAC-SHA256(canonical))` over `METHOD\nPATH\nQUERY\nTIMESTAMP\nNONCE\nBODY_HASH\nAPI_KEY` (QUERY alpha-sorted, no `?`; BODY_HASH = lowercase hex sha256 of the raw body). Gotchas: the secret is used as raw UTF-8 bytes (do NOT base64-decode it despite the `...=` look); canonical PATH must match the request URL exactly (derive both from one place); timestamp within ±5 min of Smoobu's clock. Endpoint paths are unchanged (`/api/reservations...`, incl. send-message-to-guest) — only auth changed. Signers: `smoobuFetch()` in `workers/src/smoobuApi.ts`; `SmoobuWebhook::smoobuApiRequest()` (PHP, reused by `manual_sync.php`/`lock_migration.php`). Credentials: worker secrets `SMOOBU_API_KEY` (the `usr_live_...` key id) + `SMOOBU_API_SECRET`; PHP `config['smoobu']['api_key']` + `['api_secret']`.
